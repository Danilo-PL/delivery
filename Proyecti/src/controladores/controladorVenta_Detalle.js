...
exports.listar = async (req, res)=>{
    ...
    try {
        await ModeloVenta_Detalle.findAll()
        .then(((data)=>{
            contenido.tipo=1;
...
exports.guardar = async (req, res) => {
    ...
    if(contenido.msj.length>0){
        ...
        try {
            await ModeloVenta_Detalle.create({...req.body})
            .then((data)=>{
                contenido.tipo=1;
                contenido.datos=data;
                enviar(200, contenido, res);
...
exports.editar = async (req, res) => {
    ...
    if(contenido.msj.length>0){
        ...
        try {
            await ModeloVenta_Detalle.update(
                {...req.body},
                {where: {id: id}})
            .then((data)=>{
                contenido.tipo=1;
...
exports.eliminar = async (req, res) => {
    ...
    if(contenido.msj.length>0){
        ...
        try {
            await ModeloVenta_Detalle.destroy({where: {id: id}})
            .then((data)=>{
                contenido.tipo=1;
                contenido.datos=data;
                enviar(200, contenido, res);
